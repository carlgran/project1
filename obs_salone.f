      !program test_obs
	  subroutine test_obs(wt,q2t,OBSR)
	  
	  real*8 OBSR(65),parf(188)
	  ...
	  eps=0.95
	  lmx=2
	  call get_obs(wt,q2t,thi,phii,eps,isn,parf,OBSR,lmx)
	  ...
	  implicit none
	  integer imp,kj,iv,iem,isn,k_j,i_v,il,lmx
	  real*8 wt,q2t,th,phi,rpi,thi,phii,eps
	  complex*8 m_gam(4,5,13,3,2),m_gam_pi0p(2,5,3),m_gam_pipn(2,5,3)
...
66    end 
     ...
     >           H1,H2,H3,H4,H5,H6,OBSR,lmx)       
	  ...
	  real*8 th,SQ2,rpi,phi,eps,epsi
	  complex*8 Ep(0:5),Em(0:5),Mp(0:5),Mm(0:5),Lp(0:5),Lm(0:5)
	  complex*8 H1,H2,H3,H4,H5,H6
     ...
     > RTLsn,OBSR(65)
      ...
      CONV2 = sqrt(Q2)*2.*M_P/(WCM**2+Q2-M_P**2)
      ...
      DSG=dsT+eps*dsL+sqrt(2.*eps*(1.+eps))*dsTL*cos(phi)
     >  +eps*dsTT*cos(2.*phi) 
      ...
      A0Y=-sqrt(2.*eps*(1.+eps))*RLT00/(RT00+eps*(RL00+RTT00))
      A0YP=(dsTLP/DSG)*sqrt(2.*eps*(1.-eps))
      dTLs=dsT+eps*dsL
      PXh=-sqrt(2.*eps*(1.-eps))*RLTP0X/(RT00+eps*RL00)
      PY=-sqrt(2.*eps*(1.-eps))*RLT0Y/(RT00+eps*RL00)
      PZh=-sqrt(1.-eps**2)*RTTP0Z/(RT00+eps*RL00)
      RTLs=RT00+eps*RL00
      RTLsn=RTYP0-eps*RL0Y
      
       OBSR(1:65)=(/RT00,RT0Y,RTYP0,RTXPX,RTXPZ,RTZPX,RTZPZ,RL00,  
     ...
     >         ic,kj,iv,iem,iln,par_cnt(13,5,4,3),maxe,ilc 
      common a_gami,TGa,cme,par_cnt
...
666   end
     ...
     >        ipar,ir,jr,il,ilt,jlp,jlm,par_cnt(13,5,4,3) 
	  real*8 par(*)
	  real mBW_F
...
667   end
	  ...
	  real function mBW_f(il,x) !altered Blatt Weisskopff factors
	  real*8 x
	  if(il.eq.0)mBW_f=1.
      ...
      implicit none
	  integer i_c,k_j,i_v,i_en,i_j,ic,kj,iv,ien,iq2,j,ic1,ic2,ie_m,ict
...
100   close(1)
	  ...
	  read(2,*,end=111)ic,wt,kj,iv,iem,chsq,npar,parv(1:npar)
	  if(kj.eq.1 .and. iv.eq.2)goto112
	  if(kj.eq.1 .and. iv.eq.4)goto112
	  par_cnt(ic,kj,iv,iem)=npar
112   enddo
